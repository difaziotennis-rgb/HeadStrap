...
export async function POST(request: Request) {
  try {
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: "Tennis Lesson",
              description: (() => { const wh = Math.floor(Number(hour)); const mn = Math.round((Number(hour) - wh) * 60); const dh = wh === 0 ? 12 : wh > 12 ? wh - 12 : wh; return "Private lesson on " + date + " at " + dh + ":" + String(mn).padStart(2, "0") + " " + (Number(hour) >= 12 ? "PM" : "AM"); })(),
            },
    ...
    // ALWAYS return both sessionId and url
    return NextResponse.json({ 
